...
通过 8 项检测与下载域名黑名单加成对访问的网站进行实时安全评估。当总分达到 100 分阈值时，自动触发页面风险覆盖层、桌面通知和下载拦截。
...
- **阈值分层** — ≥80 分激活下载确认弹窗，≥100 分激活完整防护（注入拦截 + 页面风险覆盖层 + 图标变红）
- **页面风险覆盖层** — 高风险时覆盖当前网页，展示风险评分、站点信息、检测原因，并支持离开网站或跳转官方网站
...
评分 ≥ 100            → 取消下载 + 确认窗口 + 注入页面级拦截 + 页面风险覆盖层 + 红色图标
```
...
- **页面风险覆盖层**：以 Shadow DOM 注入响应式玻璃覆盖层，阻断页面操作并展示风险处置选项
...
- 评分 ≥100 → 完整拦截器 + 页面风险覆盖层 + 桌面通知 + 红色图标
...
规则四  链接分析            +70 ──→  初步总分
规则五  代码工程化          +60 ──→       │
域名年龄评分                +60 ──→       ├── ≥ 100 → 红色徽章 + 桌面通知 + 页面风险覆盖层 + 注入拦截
