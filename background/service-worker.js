...
async function triggerWarningFlow(tabId, tabState) {
  ...
  // 4. 页面风险覆盖层：独立于下载拦截，确保高风险页面始终可见警告
  if (settings.showWarningWindow !== false) {
    await injectWarningOverlay(tabId, tabState);
  }
...
async function injectWarningOverlay(tabId, tabState) {
  ...
  try {
    ...
    console.error('[ServiceWorker] 注入页面风险覆盖层失败:', e);
  }
}
