  ...
  static detectSpoof(hostname) {
    ...
    const _evaluate = (labels, source) => {
        ...
        for (const segs of labelSegments) {
          for (const seg of segs) {
            if (seg === kw) {
              ...
              return _build(entry, 'strong', 'segment_exact_match',
      ...
      for (const py of pinyinKeywords) {
        for (const segs of labelSegments) {
          for (const seg of segs) {
            if (seg === py) {
              const entryList = keywordToEntries.get(py);
              if (!Array.isArray(entryList) || entryList.length === 0) continue;
              const entry = entryList[0];
              return _build(entry, 'strong', 'pinyin_exact_match',
        ...
        for (const segs of labelSegments) {
          for (const seg of segs) {
            if (seg !== kw && (seg.startsWith(kw) || seg.endsWith(kw))) {
              return _build(entry, 'weak', 'short_keyword_boundary',
                `段 "${seg}" 边界含极短品牌词 "${kw}"（B2）` + suffix);
            }
          }
        }
      }
      ...
      if (kw.length >= 5) {
        for (const label of labels) {
          if (label.includes(kw)) {
            ...
            if (!Array.isArray(entryList) || entryList.length === 0) continue;
            const entry = entryList[0];
            return _build(entry, 'weak', 'substring_include',
          ...
          if (kw.length === 5 && label.length > kw.length + 2) {
            ...
            if (label.startsWith(head4) || label.endsWith(tail4)) {
              ...
              if (!Array.isArray(entryList) || entryList.length === 0) continue;
              const entry = entryList[0];
              return _build(entry, 'weak', 'short_5char_headtail',
                `5位关键词首尾4位重合高仿：${label} 近似 ${kw}` + suffix);
            }
          }
        }
      }
    ...
    /* 规则D：高仿拼写模糊检测（三段定点采样版）
        ...
        if (isFake) {
          ...
          if (!Array.isArray(entryList) || entryList.length === 0) continue;
          const entry = entryList[0];
          return _build(entry, 'strong', 'typosquat_sample',
            `采样片段命中高仿：域名段 "${label}" 近似关键词 "${kw}"` + suffix);
          }
        }
      }
      ...
      const _typosquat = (target, kw, matchedSeg) => {
        ...
        const entryList = keywordToEntries.get(kw);
        if (!Array.isArray(entryList) || entryList.length === 0) return null;
        const entry = entryList[0];

        return _build(entry, 'strong', 'typosquat',
        ...
        for (const segs of labelSegments) {
          for (const seg of segs) {
            if (seg === kw) {
              ...
              if (!Array.isArray(entryList) || entryList.length === 0) continue;
              const entry = entryList[0];
              return _build(entry, 'weak', 'segment_exact_match',
      ...
      for (const label of labels) {
        if (shortKeywords.has(label)) {
          const entryList = keywordToEntries.get(label);
          if (!Array.isArray(entryList) || entryList.length === 0) return null;
          const entry = entryList[0];
          return _build(entry, 'weak', 'segment_exact_match',
